...
def get_multilabel_sample(batch_size, num_frames, max_num_speakers, max_simult_speakers=0, device=None):
    ...
    sample = torch.randint(0,2,(batch_size, num_frames, max_num_speakers), device=device)
...
def test_multi_to_mono_class_count():
    ...
    for max_speakers in range(1,MAX_SPEAKERS_TO_TEST):
        for max_simult_speakers in range(1, max_speakers):
            ...
            sample_mono = multilabel_to_monolabel_torch(sample, max_speakers, max_simult_speakers)

            assert get_monolabel_class_count(max_speakers, max_simult_speakers) == sample_mono.shape[-1]
...
def test_multi_to_mono_to_multi():
    ...
    sample = get_multilabel_sample(BATCH_SIZE,NUM_FRAMES,MAX_SPEAKERS,MAX_SIMULT_SPEAKERS)
    sample_mono = multilabel_to_monolabel_torch(sample, MAX_SPEAKERS, MAX_SIMULT_SPEAKERS)
    sample_multi = monolabel_to_multilabel_torch(sample_mono, MAX_SPEAKERS, MAX_SIMULT_SPEAKERS)
...
def test_multi_to_mono_too_many_simult():
    ...
    sample = get_multilabel_sample(BATCH_SIZE,NUM_FRAMES,MAX_SPEAKERS,MAX_SIMULT_SPEAKERS_IN_SAMPLE)
    sample_mono = multilabel_to_monolabel_torch(sample, MAX_SPEAKERS, MAX_SIMULT_SPEAKERS)
    sample_multi = monolabel_to_multilabel_torch(sample_mono, MAX_SPEAKERS, MAX_SIMULT_SPEAKERS)
    assert sample.shape == sample_multi.shape
